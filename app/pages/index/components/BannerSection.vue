<template>
    <section class="banner-section" ref="bannerRef">
    ...
    layers.map((layer) => {
        layer.resources.map((resource, resourceKey) => {
            if (!/\.(webm|mp4)$/.test(resource.src)) {
                ...
                // 使用导入的资源对象获取正确的 URL
                const imgUrl = bannerAssets[fileName];
                ...
                const fileName = resource.src.split('/').pop();
                const videoUrl = bannerAssets[fileName];
